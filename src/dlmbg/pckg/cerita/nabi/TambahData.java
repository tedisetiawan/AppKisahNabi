...
public class TambahData extends Activity {
    ...
    private EditText EdNamaNabi,EdKeterangan,EdGambar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EdNamaNabi = (EditText) findViewById(R.id.ed_nama_nabi);
		...
		if (savedInstanceState == null) {
			...
			if (extras != null && extras.containsKey(DataNabi.EXTRA_ROWID)) {
				id = extras.getLong(DataNabi.EXTRA_ROWID);
			}
			else
			{
	...
	private void pindahData() {
		if (id != null) {
			Cursor cursor = sqliteDB.bacaDataTerseleksiNabi(id);
			EdNamaNabi.setText(cursor.getString(1));
	...
	private void simpan() {
		String nama_nabi_in = EdNamaNabi.getText().toString();
		String keterangan_in = EdKeterangan.getText().toString();
		String gambar_in = EdGambar.getText().toString();

		if (id != null) {
			sqliteDB.updateData(id, sqliteDB.ambilDataNabi(nama_nabi_in, keterangan_in, gambar_in),"tbl_nabi","_id");
		}
		else {
